...
---Runs a iteratee functions for every element inside the collection **in series** to pass a new collection to result_callback. Passes the new mapped collection to result_callback. **Errors may stop the iteration early**
---@param collection any[] Collection of elements to pass as an argument to iteratee map function
---@param iteratee fun(element: any, callback: fun(err?: any, value?: boolean), index?: number, collection: any[]) A map function that runs with each element from the collection, can pass new value to final list on result_callback in series via its callback. When iteratee callback passes nil as result, the result gets converted to immutable & empty null objects to keep the table length intact in lua.
...
return function(collection, iteratee, result_callback)
  if type(iteratee) ~= "function" then
    return error('Callback.map_limit(collection, iteratee, callback): Expected "iteratee" to be a function')
  end

  return map(1, collection, iteratee, result_callback)
end
