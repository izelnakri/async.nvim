...
---Runs a truth test iteratee filter function with every element inside the collection **in series**. Passes the filtered result to result_callback. **Errors may stop the iteration early**
...
return function(collection, iteratee, result_callback)
  if type(iteratee) ~= "function" then
    return error('Callback.filter_series(collection, iteratee, callback): Expected "iteratee" to be a function')
  end

  return filter(1, collection, iteratee, result_callback)
end
