...
return function(collection, limit, iteratee, result_callback)
  if type(iteratee) ~= "function" then
    return error('Callback.map_limit(collection, limit, iteratee, callback): Expected "iteratee" to be a function')
  end

  return map(limit, collection, iteratee, result_callback)
end
